...
static struct rte_mbuf * ng_send(struct rte_mempool *mbuf_pool, unsigned char * data, uint16_t len){
    //从mempool获取mbuf，使用dpdk内存池最小的单位是mbuf
    const unsigned total_len = len + 14 + 20 + 8;
		...
		for (i = 0;i < num_recvd;i ++) {
			...
			if (iphdr->next_proto_id == IPPROTO_UDP) {
				...
				*((char*)udphdr + length) = '\0';
				...
				struct in_addr addr;
				addr.s_addr = iphdr->src_addr;
				printf("src: %s:%d, %d, ", inet_ntoa(addr), ntohs(udphdr->src_port));
