...
![image-20220429165305786](.assets/image-20220429165305786.png)
...
server端处于`LISTEN`状态，当收到`SYN`包时，使用`ng_tcp_handle_listen`函数来处理，里面最重要的是处理：

* 接收报文的`tcphdr->sent_seq; tcphdr->recv_ack;`
* server端维护的`stream->send_nxt; stream->recv_nxt;`
* 发送报文的`fragment->seqnum; fragment->acknum;`

> `stream->recv_nxt = ntohl(tcphdr->sent_seq) + 1; `
>
> `fragment->seqnum = stream->send_nxt;`
>
> `fragment->acknum = strean->recv_nxt`
...
### 1 tcp数据传输之ack与seqnum的确认实现

### 2 tcp数据传输之ack与seqnum代码实现

这两节课主要实现了server端TCP连接处于`ESTABLISHED`时候，收到client发来的数据包，会产生三个包，其中

* 一个丢给上层应用程序；
* 一个组成TCP数据报打上`ACK`标签回复client；
* 一个复制client发来的数据，组TCP报文打上`ACK | PUSH`标签回复client；

上述三个包都是由函数`ng_tcp_handle_established`处理的，但是个人认为，第三个包没有逻辑，后期代码可能会删除。所以本节不展示`ng_tcp_handle_established`代码，重点研究`stream->send_nxt; stream->recv_nxt`与`tcphdr—>sent_seq; tcphdr->ack`之间的关系。

#### 2.1 函数`ng_tcp_handle_established`

当TCP连接处于`ESTABLISHED`时候，可能会接收到4种包`ACK`包，`PUSH`包，`FIN`包，`SYN`包；其中`PUSH`包可以理解成需要立即推给应用程序来处理的数据包，内核收到这种包，意思就是不用等后面的数据包了，直接给应用程序处理；如果没有`PUSH`可能会等后续包到来，组包之后再发给应用程序。

![image-20220429163128526](.assets/image-20220429163128526.png)

结合上图，server端维护的`stream->recv_nxt`字面意思理解就是，下一个收到的数据包，跟server端回复client的包`tcphdr->recv_ack`其实是一个意思，代码中`recv_nxt`也是给TCP包头`recv_ack`赋值用的。

```c
if(stream->recv_nxt != ntohs(tcphdr->sent_seq)) { //出现duplicated ack
	......
}
...
### 3 tcp协议api实现之`bind`，`listen`的实现

#### 3.1 `tcp posix api`实现`bind`

新疆的地形**三山夹两盆**跟组织架构图神似；

#### 3.2 `tcp posix api`实现`listen`



### 4 tcp协议api实现之`accept`的实现



### 5 tcp协议api实现之`send,recv`的实现



### 6 tcp协议api实现之`close`的实现
...
### 8 tcp协议调试之ringbuffer内存错误

启动`./build/dpdk_tcp_api`程序，然后使用net assistant与其建立TCP连接，一切正常，然后断开连接，再次尝试建立TCP连接，会报如下错误：

```bash
RING: Cannot reserve memory
RING: Cannot reserve memory
ng_tcp_stream_create:1434: rte_ring_create failed
tcp---->src:10.164.16.14:50580---> dst:10.164.16.40:8899

Program received signal SIGSEGV, Segmentation fault.

```

问题原因是：net assistant主动断开连接之后，并没有发出第四次挥手的ack包，而是等了120秒之后才发出一个rst包，这就导致了connfd所在的stream并没有释放内存，释放stream内存函数是`ng_tcp_handle_last_ack`;如果不发第4次ack包，该函数就一直不会被调用。

此时尝试创建新TCP连接，函数`ng_tcp_stream_create`会再次创建新的connfd所在的stream的时候，下面代码会创建相同的ring名字`sndbuf; rcvbuf`而报错。
...
![image-20220505162941308](.assets/image-20220505162941308.png)

所以在第3次挥手的包应该是`FIN | ACK`包
...
pthread_mutex_lock与pthread_cond_wait联合使用，pthread_mutex_lock与pthread_cond_wait结合以后，阻塞点转移到pthread_cond_wait，此时需要依靠pthread_cond_signal唤醒，另外tcp_server_entry线程中两个while循环，也跟这个机制有关。
