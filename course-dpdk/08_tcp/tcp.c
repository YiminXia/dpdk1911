...
static int udp_server_entry(__attribute__((unused))  void *arg) {
    ...
    localaddr.sin_addr.s_addr = inet_addr("10.164.16.24"); // 0.0.0.0
...
/* 函数名：ng_stream_search
 * 入参：  五元组，都是输入参数,protocol先不用，默认都是IPPROTO_TCP
 * 返回值：struct ng_tcp_stream *指针
 * 作用：  在ng_tcp_stream组成的双向链表中，按照五元组这个唯一索引查找
 */
static struct ng_tcp_stream * ng_stream_search(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport){
...
static struct ng_tcp_stream * ng_stream_create(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport) {
...
static int ng_tcp_process(struct rte_mbuf *tcpmbuf) {
    ...
    if(stream == NULL) {
        stream = ng_stream_create(iphdr->src_addr, iphdr->dst_addr,
            tcphdr->src_port, tcphdr->dst_port);
    }
    ...
    switch (stream->status) {
        ...
        case NG_TCP_STATUS_FIN_WAIT_1:      // 暂定client,谁先主动断开，server | client,本代码中是由net assistant主动关闭的，所以暂定client
            break;
